...
def install_packages():
    try:
        ...
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "plotly==5.18.0", "--break-system-packages"])
            subprocess.check_call([sys.executable, "-m", "pip", "install", "pyteomics==4.6.2", "--break-system-packages"])
            ...
            st.info("If the error is related to permissions, try running the following command manually:")
            st.code("sudo python3 -m pip install plotly==5.18.0 pyteomics==4.6.2")
            st.stop()
        return False
